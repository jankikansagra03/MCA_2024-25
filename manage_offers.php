            ...
            <div class="col-lg-6 offset-lg-6 col-12">
                <div class="row">
                    ...
                    <div class="col-md-4 col-12 text-md-right text-center">
                        <a href="admin_add_offer.php" class="btn btn-dark form-control form-control-md">
                            <i class="fa-solid fa-list"></i> &nbsp; Add Offer
                        </a>
                    </div>
                </div>
            </div>
        ...
        <div class="col-12">
            <div class="table-responsive">
                ...
                <table class="table table-striped">
                    ...
                    $search_query = '';
                    if (!empty($search)) {
                        $search_query = "WHERE offer LIKE '%$search%' OR status LIKE '%$search%' OR discount_percentage LIKE %$search%";
                    }

                    // Get the total number of records matching the search query
                    $query = "SELECT * FROM offers $search_query";
                    ...
                    $query = "SELECT * FROM offers $search_query LIMIT $start_from, $records_per_page";
                    $result = mysqli_query($con, $query);
                    ?>
                    ...
                    <tbody id="myTable">
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            echo "<td>";
                            echo "<a href='admin_edit_offer.php?id=" . $row['id'] . "' class='btn btn-warning'><i class='fas fa-edit'></i> Edit</a>&nbsp&nbsp;";

                            if ($row['status'] == 'Active') {
                                echo "<a href='admin_toggle_offer_status.php?id=" . $row['id'] . "&status=Inactive' class='btn btn-success'><i class='fas fa-toggle-on'></i> Deactivate</a>&nbsp&nbsp;";
                            } else {
                                echo "<a href='admin_toggle_offer_status.php?id=" . $row['id'] . "&status=Active' class='btn btn-dark'><i class='fas fa-toggle-off'></i> Activate</a>&nbsp&nbsp;";
                            }
